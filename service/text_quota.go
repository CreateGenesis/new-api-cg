...
func calculateTextQuotaSummary(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.Usage) textQuotaSummary {
	...
	if summary.CacheUsageValidationSplit {
		...
		summary.IsClaudeUsageSemantic = summary.UsageSemantic == UsageSemanticAnthropic
		sourceUsage := *usage
		if sourceUsage.UsageSemantic == "" {
			sourceUsage.UsageSemantic = summary.UsageSemantic
			if isLegacyClaudeDerivedOpenAIUsage(relayInfo, usage) {
				sourceUsage.UsageSemantic = UsageSemanticAnthropic
			}
		}
		if usage.UsageSemantic == "" && relayInfo.ChannelMeta != nil && relayInfo.ChannelType == constant.ChannelTypeOpenRouter {
			sourceUsage.UsageSemantic = UsageSemanticOpenAI
		}
		if isLegacyClaudeDerivedOpenAIUsage(relayInfo, usage) {
			summary.UsageSemantic = UsageSemanticAnthropic
			summary.IsClaudeUsageSemantic = true
		}
		normalizedUsage := NormalizeUsageForSemantic(&sourceUsage, summary.UsageSemantic)
	...
	if isOpenRouterClaudeBilling {
		...
		if summary.CacheCreationTokens == 0 && relayInfo.PriceData.CacheCreationRatio != 1 && usage.Cost != 0 && !isUsingCustomSettings {
			openAIUsage := NormalizeUsageForSemantic(usage, UsageSemanticOpenAI)
			maybeCacheCreationTokens := CalcOpenRouterCacheCreateTokens(openAIUsage, relayInfo.PriceData)
			if maybeCacheCreationTokens >= 0 && openAIUsage.PromptTokens >= maybeCacheCreationTokens {
...
func PostTextConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.Usage, extraContent []string) {
	...
	streamInterruptionBilling := EvaluateStreamInterruptionBilling(relayInfo, summary.CompletionTokens, summary.Quota)
	summary.Quota = streamInterruptionBilling.FinalQuota
	...
	if !summary.hasBillableUsage() && !streamInterruptionBilling.Applied {
	...
	if relayInfo.KimiK3BillingAudit != nil {
		other.SetAdmin("kimi_k3_billing", relayInfo.KimiK3BillingAudit)
	}
	if summary.CacheUsageValidationSplit {
	...
	if summary.CacheUsageValidationSplit && billingUsage != nil {
		other.SetPublic("input_tokens_total", summary.UsageNormalization.InputTokens.TotalInputTokens)
	...
	attachStreamInterruptionBilling(ctx, relayInfo, other, streamInterruptionBilling)
