...
func ConvertTNTTencentClaudeRequest(request *dto.ClaudeRequest) (*dto.GeneralOpenAIRequest, error) {
	...
	tools, err := common.Any2Type[[]dto.Tool](request.Tools)
	if err == nil {
	...
	if request.ToolChoice != nil {
		if toolChoice, err := common.Any2Type[dto.ClaudeToolChoice](request.ToolChoice); err == nil {
...
func SanitizeTNTTencentChatRequest(request *dto.GeneralOpenAIRequest) error {
	...
	for index := range request.Messages {
		...
		if len(toolCalls) > 0 {
			encoded, err := common.Marshal(toolCalls)
			if err != nil {
				return err
			}
	...
	if request.ResponseFormat != nil && len(request.ResponseFormat.JsonSchema) > 0 {
		var schema any
		if err := common.Unmarshal(request.ResponseFormat.JsonSchema, &schema); err == nil {
...
func tntClaudeMessageToChat(message dto.ClaudeMessage) []dto.Message {
	...
	if message.Role == "assistant" {
		...
		if len(toolCalls) > 0 {
			...
			assistant.ToolCalls, _ = common.Marshal(toolCalls)
		}
