...
func updateBatchTasks(ctx context.Context, adaptor BatchTaskPollingAdaptor, channelId int, taskIds []string, taskM map[string]*model.Task) error {
	...
	info.ChannelMeta = &relaycommon.ChannelMeta{ChannelBaseUrl: baseURL, ChannelSetting: ch.GetSetting()}
	info.ApiKey = ch.Key
	info.HeadersOverride = ch.GetHeaderOverride()
	adaptor.Init(info)
...
func updateVideoTasks(ctx context.Context, platform constant.TaskPlatform, channelId int, taskIds []string, taskM map[string]*model.Task) error {
	...
	info.HeadersOverride = cacheGetChannel.GetHeaderOverride()
	adaptor.Init(info)
