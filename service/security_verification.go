...
func BindVerificationOperation(operation VerificationOperation) (VerificationBinding, error) {
	...
	case VerificationScopePasskeyRegister, VerificationScopePasskeyDelete, VerificationScopeTwoFASetup,
		...
		constant.SecurityProofScopeSystemBackupExport, constant.SecurityProofScopeSystemBackupImport:
		if len(fields) != 0 {
			return VerificationBinding{}, ErrVerificationContextInvalid
		}
		normalized = struct{}{}
...
func securityVerificationPolicy(scope string, state model.UserVerificationState) ([]VerificationMethodOption, error) {
	...
	switch scope {
	case constant.SecurityProofScopeSystemBackupExport, constant.SecurityProofScopeSystemBackupImport:
		if state.Role != common.RoleRootUser || !state.HasPassword {
...
func VerifySecurityInput(identity AuthIdentity, input VerificationInput) (*SecurityProof, error) {
	...
	case VerificationMethodPassword:
		...
		if input.Scope == constant.SecurityProofScopeSystemBackupExport || input.Scope == constant.SecurityProofScopeSystemBackupImport {
			if user.Role != common.RoleRootUser || user.Status != common.UserStatusEnabled || subtle.ConstantTimeCompare([]byte(strings.TrimSpace(input.Username)), []byte(user.Username)) != 1 {
				return nil, ErrVerificationFailed
			}
		}
