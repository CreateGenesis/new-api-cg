...
func registerChannelRoutes(apiRouter *gin.RouterGroup) {
	...
	channelRoute.POST("/:id/multi_key/:key_index/key",
		...
		middleware.CriticalRateLimit(),
		middleware.DisableCache(),
		controller.GetMultiKeyChannelKey,
	)
...
var channelPermissionRoutes = []permissionRoute{
	{method: http.MethodGet, path: "/:id/group_scheduling/probes", permission: authz.ChannelRead, handler: controller.GetGroupSchedulingStatus},
