...
func TestProxyTaskMediaAppliesChannelRewriteOnlyWithCredentials(t *testing.T) {
	task := setupGenericTaskTest(t)
	task.PrivateData.Key = "private-task-key"
	previousCache := common.MemoryCacheEnabled
	common.MemoryCacheEnabled = false
	t.Cleanup(func() { common.MemoryCacheEnabled = previousCache })
	require.NoError(t, model.DB.Model(&model.Channel{}).Where("id = ?", task.ChannelId).Update("header_override", `{"Authorization":"Bearer {api_key}","X-Route":"artifact"}`).Error)
	var authorization, route string
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	defer upstream.Close()
	allowPrivateTaskMediaTest(t)
	for _, credentialless := range []bool{false, true} {
		c, _ := gin.CreateTestContext(httptest.NewRecorder())
		c.Request = httptest.NewRequest(http.MethodGet, "/content", nil)
		require.NoError(t, proxyTaskMedia(c, task, &relaychannel.TaskContentRequest{URL: upstream.URL, Method: http.MethodGet, Credentialless: credentialless}))
		if credentialless {
			assert.Empty(t, authorization)
			assert.Empty(t, route)
		} else {
			assert.Equal(t, "Bearer private-task-key", authorization)
			assert.Equal(t, "artifact", route)
		}
	}
}
