...
func proxyTaskMedia(c *gin.Context, task *model.Task, descriptor *relaychannel.TaskContentRequest) error {
	...
	if proxy != "" {
		client, err = service.GetHttpClientWithProxyFallback(proxy, channel.GetSetting().ProxyFallbackDirect)
		if err != nil {
			return &taskMediaProxyError{
	...
	if !descriptor.Credentialless {
		...
		if err := relaycommon.ResolveAndApplyHeaderRewriteToRequest(req, relaycommon.HeaderRewriteInput{
			ChannelSetting: channel.GetSetting(), LegacyOverride: channel.GetHeaderOverride(), APIKey: key,
		}); err != nil {
			return &taskMediaProxyError{status: http.StatusBadGateway, code: "artifact_request_rejected", message: "Artifact request headers were rejected", err: err}
		}
	}
