...
func buildTestLogOther(c *gin.Context, info *relaycommon.RelayInfo, priceData hosttypes.PriceData, usage *dto.Usage, tieredResult *billingexpr.TieredResult) *model.LogOther {
	...
	if input.CacheCreationInputTokens > 0 {
		other.SetPublic("cache_creation_tokens", input.CacheCreationInputTokens)
		other.SetPublic("cache_creation_ratio", priceData.CacheCreationRatio)
		other.SetPublic("cache_write_tokens", input.CacheCreationInputTokens)
	}
	if input.CacheCreation5mInputTokens > 0 {
		other.SetPublic("cache_creation_tokens_5m", input.CacheCreation5mInputTokens)
		other.SetPublic("cache_creation_ratio_5m", priceData.CacheCreation5mRatio)
	}
	if input.CacheCreation1hInputTokens > 0 {
		other.SetPublic("cache_creation_tokens_1h", input.CacheCreation1hInputTokens)
		other.SetPublic("cache_creation_ratio_1h", priceData.CacheCreation1hRatio)
	}
	other.SetPublic("input_tokens_total", input.TotalInputTokens)
