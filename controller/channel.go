...
func applyFetchModelsHeaderOverrides(channel *model.Channel, key string, headers http.Header) error {
	...
	return relaychannel.ApplyHeaderRewrite(headers, info, nil)
}
...
type AddChannelRequest struct {
	...
	MultiKeyCacheAffinityThresholdPercent *int                  `json:"multi_key_cache_affinity_threshold_percent"`
...
func AddChannel(c *gin.Context) {
	...
	case "multi_to_single":
		...
		addChannelRequest.Channel.ChannelInfo.MultiKeyCacheAffinityThresholdPercent = addChannelRequest.MultiKeyCacheAffinityThresholdPercent
...
func UpdateChannel(c *gin.Context) {
	...
	if channel.MultiKeyCacheAffinityThresholdPercent != nil && channel.ChannelInfo.IsMultiKey {
		threshold := *channel.MultiKeyCacheAffinityThresholdPercent
		channel.ChannelInfo.MultiKeyCacheAffinityThresholdPercent = &threshold
	}
...
func applyFetchModelsRequestOverrides(channel *model.Channel, req fetchModelsRequest) error {
	...
	if req.Setting != nil {
		rawSetting := strings.TrimSpace(*req.Setting)
		if rawSetting != "" {
			var channelSetting dto.ChannelSettings
			if err := common.UnmarshalJsonStr(rawSetting, &channelSetting); err != nil {
				return fmt.Errorf("setting must be a JSON object: %w", err)
			}
		}
		channel.Setting = &rawSetting
	}
	if req.Proxy != nil {
