...
func formatLogOtherJSON(value string, visibility logOtherVisibility, redactErrors ...bool) string {
	if value == "" {
		return ""
	}
	...
	if visibility == logOtherVisibilityUser {
		sensitiveKeys := []string{"is_model_mapped", "upstream_model_name"}
		if len(redactErrors) > 0 && redactErrors[0] {
			sensitiveKeys = append(sensitiveKeys, "error_type", "error_code", "status_code", "request_path")
		}
