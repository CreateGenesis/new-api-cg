...
func (c *ChannelInfo) Scan(value interface{}) error {
	...
	if normalized, err := NormalizeMultiKeyLeastRequestsWindowSeconds(c.MultiKeyLeastRequestsWindowSeconds); err == nil {
		c.MultiKeyLeastRequestsWindowSeconds = normalized
	} else {
...
func (channel *Channel) getNextEnabledKeyWithSelection(affinityValue string, excludedIndexes map[int]struct{}, persistAffinity bool, preferredDigests []string) (string, int, *types.NewAPIError) {
	...
	case constant.MultiKeyModePolling:
		...
		available := make(map[int]struct{}, len(enabledIdx))
...
func (channel *Channel) ValidateSettings() error {
	...
	if _, err := common.ParseProxyURLsStrict(channelParams.Proxy); err != nil {
	...
	if channelOtherSettings.ResponseHeaderTimeout != nil {
		if err := channelOtherSettings.ResponseHeaderTimeout.Validate(); err != nil {
			return fmt.Errorf("response_header_timeout.%w", err)
		}
	}
