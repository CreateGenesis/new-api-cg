...
// then task_plugin_identity, then video_understanding) for error attribution.
...
// error) and dropped for filters that require channel settings.
...
func channelMatchesFilter(ch *Channel, modelName string, filter dto.ChannelFilter) bool {
	switch filter.Kind {
	case dto.FilterVideoUnderstanding:
		return !ch.GetOtherSettings().DisableVideoUnderstanding
