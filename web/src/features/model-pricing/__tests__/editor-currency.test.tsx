...
it('converts direct request price overrides between site currency and stored USD', async () => {
  const editor = renderEditor({
    billingMode: 'tiered_expr',
    billingExpr: 'tier("base", p * 2 + c * 4)',
    requestRuleExpr: buildRequestRuleExpr([
      {
        conditions: [
          { source: 'header', path: 'x-mode', mode: 'eq', value: 'edit' },
        ],
        action: 'override',
        multiplier: '',
        override: { label: 'edit', input_unit_cost: 3, output_unit_cost: 6 },
      },
    ]),
  })
  const before = await commit(editor.ref)
  ...
  fireEvent.change(inputs[1], { target: { value: '28' } })
  const saved = await commit(editor.ref)
  const groups = tryParseRequestRuleExpr(saved?.requestRuleExpr ?? '')
  expect(groups?.[0].override?.input_unit_cost).toBe(4)
  expect(groups?.[0].override?.output_unit_cost).toBe(6)
})
