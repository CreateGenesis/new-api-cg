...
export function ModelMappingEditor(props: ModelMappingEditorProps) {
  ...
  return (
    <div
      className='space-y-2'
      role='group'
      aria-label={props.ariaLabel || t('Model Mapping')}
    >
      <Tabs value={mode} onValueChange={handleModeChange} className='space-y-2'>
          ...
          {rows.length > 0 ? (
            <div className='space-y-2'>
              <div className='grid grid-cols-[1fr_1fr_auto] gap-2 text-sm font-medium'>
                <div>{props.sourceLabel || t('Original Model')}</div>
                <div>{props.targetLabel || t('Replacement Model')}</div>
                <div className='w-10' />
              </div>
              {rows.map((row) => (
                <div
                ...
                >
                  <Input
                    aria-label={props.sourceLabel || t('Original Model')}
                    value={row.from}
                    onChange={(e) =>
                  ...
                  />
                  <Input
                    aria-label={props.targetLabel || t('Replacement Model')}
                    value={row.to}
                    onChange={(e) =>
        ...
        <TabsContent value='json'>
          <JsonCodeEditor
            ...
            ariaLabel={props.ariaLabel || t('Model Mapping')}
          />
        </TabsContent>
      </Tabs>
