...
Direct request-price overrides generated by the editor use the explicit
`p_total`, `c_total`, and `*_total` token values. They are raw totals scoped to
that override; the generated expression subtracts only the override's own
separately-priced categories. This keeps an unmatched override from changing
the base expression's `p`/`c` remainder.
...
| `multipart_param` | `multipart_param(path) → any` | Reads explicit multipart field/file metadata (for example `files.input_reference.0.content_type` or `files.input_reference.#`); file content is never exposed |
| `header` | `header(key) → string` | Reads a request header value |
| `has` | `has(source, substr) → bool` | Substring check |
| `rule_override_tier` | `rule_override_tier(base, matched, override, name) → float64` | Returns the override cost when matched and records its name; otherwise returns the base cost |
...
The visual request-rule editor also supports direct price overrides. These are
compiled into the internal `rule_override_tier(base, condition, override, name)`
function, which returns the override expression directly when the condition
matches; it does not multiply the base result. Multipart rules remain explicit:
the expression must name the metadata path to inspect.
