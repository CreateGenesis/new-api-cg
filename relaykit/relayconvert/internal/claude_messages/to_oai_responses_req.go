...
func ClaudeMessagesRequestToOpenAIResponses(claudeRequest dto.ClaudeRequest, info convmeta.Meta) (*dto.OpenAIResponsesRequest, error) {
	...
	if convmeta.OptionsOf(info).Claude.ProviderManagedReasoning && effectiveEffort != "" {
		responsesRequest.Reasoning = &dto.Reasoning{Effort: string(effectiveEffort)}
	}
...
func claudeRequestReasoningIntent(claudeRequest *dto.ClaudeRequest, info convmeta.Meta) (reasoning.Intent, reasoning.Effort, error) {
	if convmeta.OptionsOf(info).Claude.ProviderManagedReasoning {
		return reasoning.Intent{}, reasoning.Effort(claudeRequest.GetEfforts()), nil
	}
