...
func (s *ClaudeToChatStreamState) ConvertChunk(claudeResponse *dto.ClaudeResponse) (*dto.ChatCompletionsStreamResponse, error) {
	...
	var incoming *dto.ClaudeUsage
	if claudeResponse.Usage != nil {
		incoming = claudeResponse.Usage
	} else if claudeResponse.Message != nil {
		incoming = claudeResponse.Message.Usage
	}
	if incoming != nil {
		s.usage = dto.MergeUsageNonZero(s.usage, UsageFromClaudeAPIUsage(incoming))
	}
...
func (s *ClaudeToChatStreamState) StreamUsage() *dto.Usage { return s.usage }
	...
	if claudeResponse.Type == "message_start" {
		...
		if claudeResponse.Message != nil && claudeResponse.Message.Usage != nil {
			...
			if claudeInfo.PreserveNormalizedAnthropicBilling {
				claudeInfo.Usage.BillingUsage = updateClaudeBillingUsage(messageUsage, claudeInfo.Usage)
			} else {
				claudeInfo.Usage.BillingUsage = claudeBillingUsageFromSemanticUsage(claudeInfo.Usage)
			}
		}
