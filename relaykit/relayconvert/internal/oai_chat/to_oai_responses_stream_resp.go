...
type ChatToResponsesStreamEvent struct {
	...
	DoneText      string `json:"-"`
	DoneArguments string `json:"-"`
...
func (s *ChatToResponsesStreamState) SetStreamUsage(usage *dto.Usage) {
	if s != nil && usage != nil {
		s.Usage = UsageForOpenAIResponses(usage)
	}
}
...
func (s *ChatToResponsesStreamState) doneDeltaEvents() []ChatToResponsesStreamEvent {
	...
	if s.textStarted && !s.textDone {
		...
		textDoneEvent := s.event("response.output_text.done", textDone)
		textDoneEvent.DoneText = s.text.String()
		events = append(events, textDoneEvent)
		events = append(events, s.event(responsesEventOutputItemDone, dto.ResponsesStreamResponse{
	...
	if s.reasoningStarted && !s.reasoningDone {
		...
		reasoningDoneEvent := s.event(responsesEventReasoningSummaryDone, reasoningDone)
		reasoningDoneEvent.DoneText = s.reasoning.String()
		events = append(events, reasoningDoneEvent)
		events = append(events, s.event(responsesEventOutputItemDone, dto.ResponsesStreamResponse{
			Type:        responsesEventOutputItemDone,
			OutputIndex: intPtr(s.reasoningIndex),
	...
	for _, tool := range s.sortedTools() {
		...
		argumentsDoneEvent := s.event(responsesEventFunctionArgsDone, argumentsDone)
		argumentsDoneEvent.DoneArguments = tool.Arguments.String()
		argumentsDoneEvent.FunctionName = tool.Name
		events = append(events, argumentsDoneEvent)
		events = append(events, s.event(responsesEventOutputItemDone, dto.ResponsesStreamResponse{
			Type:        responsesEventOutputItemDone,
			OutputIndex: intPtr(tool.OutputIndex),
