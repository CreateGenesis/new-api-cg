...
func OpenAIChatRequestToClaudeMessages(c context.Context, info convmeta.Meta, textRequest dto.GeneralOpenAIRequest) (*dto.ClaudeRequest, error) {
	...
	if opts.Claude.ProviderManagedReasoning && len(textRequest.THINKING) > 0 {
		if err := kitutil.Unmarshal(textRequest.THINKING, &claudeRequest.Thinking); err != nil {
			return nil, err
		}
	}
	if !opts.Claude.ProviderManagedReasoning {
		reasoningRequest := textRequest
		if reasoningRequest.ReasoningEffort == "none" {
			reasoningRequest.Reasoning = nil
		}
		sourceReasoning, err := reasoning.FromOpenAIChat(&reasoningRequest)
		if err != nil {
