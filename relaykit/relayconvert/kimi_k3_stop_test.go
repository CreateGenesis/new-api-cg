...
func TestKimiK3ChatStreamStopFilterUsesDecodedRequestArray(t *testing.T) {
	...
	require.NoError(t, kitutil.Unmarshal([]byte(`{"model":"kimi-k3","stop":["FIRST","SECOND"]}`), &request))
...
func TestApplyKimiK3StopToClaudeResponseOnlyTruncatesText(t *testing.T) {
	response := &dto.ClaudeResponse{
		...
		Content: []dto.ClaudeMediaMessage{
			{Type: "thinking", Thinking: kitutil.GetPointer("reasoning<STOP>")},
			{Type: "text", Text: kitutil.GetPointer("answer<STOP>ignored")},
...
func TestKimiK3ClaudeStreamStopFilterMatchesAcrossTextDeltas(t *testing.T) {
	...
	terminal := &dto.ClaudeResponse{
		...
		Delta: &dto.ClaudeMediaMessage{
			...
			PartialJson: kitutil.GetPointer(`{"value":"END"}`),
		},
	}
