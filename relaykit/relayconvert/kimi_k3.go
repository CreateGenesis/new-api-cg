...
func NormalizeKimiK3ChatRequest(request *dto.GeneralOpenAIRequest) error {
	...
	if request.MaxTokens == nil && request.MaxCompletionTokens == nil {
		request.MaxTokens = kitutil.GetPointer(kimiK3DefaultMaxTokens)
	}
	...
	if hideThinking {
		...
		if len(request.ChatTemplateKwargs) > 0 && kitutil.GetJsonType(request.ChatTemplateKwargs) == "object" {
			var kwargs map[string]any
			if err := kitutil.Unmarshal(request.ChatTemplateKwargs, &kwargs); err != nil {
			...
			if len(kwargs) == 0 {
				...
				encoded, err := kitutil.Marshal(kwargs)
				if err != nil {
					return err
				}
...
func NormalizeKimiK3ResponsesRequest(request *dto.OpenAIResponsesRequest) error {
	...
	if request.MaxOutputTokens == nil {
		request.MaxOutputTokens = kitutil.GetPointer(kimiK3DefaultMaxTokens)
	}
...
func kimiK3RawThinkingDisabled(raw json.RawMessage) bool {
	...
	if kitutil.GetJsonType(raw) == "boolean" {
		var enabled bool
		return kitutil.Unmarshal(raw, &enabled) == nil && !enabled
	}
	if kitutil.GetJsonType(raw) != "object" {
		return false
	}
	var thinking dto.Thinking
	return kitutil.Unmarshal(raw, &thinking) == nil && thinking.Type == "disabled"
}
...
func kimiK3ChatTemplateThinkingDisabled(raw json.RawMessage) bool {
	...
	var kwargs map[string]any
	if kitutil.Unmarshal(raw, &kwargs) != nil {
		return false
	}
...
func validateKimiK3ThinkingSwitches(values ...json.RawMessage) error {
	for _, raw := range values {
		...
		if err := kitutil.Unmarshal(raw, &config); err != nil {
...
func validateKimiK3Stop(stop any) error {
	...
	case []string:
		sequences = value
	default:
		converted, err := kitutil.Any2Type[[]string](stop)
		if err != nil {
...
func validateKimiK3ToolChoice(choice any) error {
	...
	choiceMap, err := kitutil.Any2Type[map[string]any](choice)
	if err != nil {
		return fmt.Errorf("invalid tool_choice")
	}
	if kitutil.Interface2String(choiceMap["type"]) == "allowed_tools" {
		return nil
	}
