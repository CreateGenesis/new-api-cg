...
func TestStreamInterruptionBillingSettingsJSONCompatibility(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.NoError(t, json.Unmarshal([]byte(tt.json), &settings))
			if tt.mode == "" {
...
func TestChannelOtherSettingsCacheUsageValidationSplitJSONCompatibility(t *testing.T) {
	...
	require.NoError(t, json.Unmarshal([]byte(`{}`), &disabled))
	assert.False(t, disabled.CacheUsageValidationSplit)

	var enabled ChannelOtherSettings
	require.NoError(t, json.Unmarshal([]byte(`{"cache_usage_validation_split":true}`), &enabled))
