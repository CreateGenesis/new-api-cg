...
func SecurityProofRequired(requiredScope string, allowedMethods []string) gin.HandlerFunc {
	return func(c *gin.Context) {
		authorization := RequireSecurityProof(c, service.VerificationOperation{Scope: requiredScope})
		if authorization == nil {
			return
		}
		if !slices.Contains(allowedMethods, authorization.Method) {
			securityProofError(c, "SECURITY_PROOF_METHOD_MISMATCH", service.ErrProofMethod.Error())
			return
		}
		c.Next()
	}
