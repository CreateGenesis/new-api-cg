...
// ApplyChannelPin copies the request's resolved task affinity onto relay state.
func ApplyChannelPin(c *gin.Context, info *relaycommon.RelayInfo) *dto.TaskError {
	return service.ApplyChannelPin(c, info)
}
...
func RelayTaskSubmit(c *gin.Context, info *relaycommon.RelayInfo) (*TaskSubmitResult, *dto.TaskError) {
	...
	if useTiered {
		...
		priceData = hosttypes.PriceData{Quota: quota, QuotaToPreConsume: quota, GroupRatioInfo: groupRatioInfo}
	...
	if err != nil {
		...
		if errors.As(err, &overloadErr) && overloadErr.GetErrorCode() == types.ErrorCodeChannelOverloaded {
			return nil, service.TaskErrorWrapperLocal(overloadErr.Err, string(types.ErrorCodeChannelOverloaded), http.StatusServiceUnavailable)
		}
