...
func TestTaskAdaptorMapsJSContract(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/tasks/upstream-1":
			...
			assert.Equal(t, "task-key", r.Header.Get("X-Key-Test"))
	...
	info.ChannelSetting.HeaderRewrite = &types.ChannelHeaderRewriteSettings{HeaderRewriteRule: types.HeaderRewriteRule{Set: map[string]string{"X-Rewrite-Test": "rewritten"}}}
	info.HeadersOverride = map[string]any{"X-Key-Test": "{api_key}"}
	adaptor.Init(info)
	...
	queryResp, err := adaptor.FetchTask(server.URL, "secret", &model.Task{
		Action:      info.Action,
		PrivateData: model.TaskPrivateData{UpstreamTaskID: parsed.UpstreamTaskID, Key: "task-key"},
	}, "")
	require.NoError(t, err)
