...
func (a *TaskAdaptor) ValidateRequestAndSetAction(c *gin.Context, info *relaycommon.RelayInfo) *dto.TaskError {
	...
	if request, exists := c.Get("task_request"); !exists || request == nil {
		if prepared, ok := c.Get(pluginruntime.ContextKeyRouteRequest); ok {
			if canonical, ok := prepared.(pluginruntime.RouteRequestContext); ok && canonical.RequestBody != nil {
				c.Set("task_request", jsonValue(canonical.RequestBody))
			}
		}
	}
	if request, exists := c.Get("task_request"); !exists || request == nil {
...
func (a *TaskAdaptor) FetchBatchTasks(baseURL, key string, tasks []*model.Task, proxy string) (*http.Response, error) {
	...
	return a.doFetchDescriptor(baseURL, key, proxy, value)
}
...
func (a *TaskAdaptor) doFetchDescriptor(baseURL, key, proxy string, value any) (*http.Response, error) {
	...
	if a.info != nil && a.info.ChannelMeta != nil {
		fallbackDirect = a.info.ChannelSetting.ProxyFallbackDirect
		if err := relaycommon.ResolveAndApplyHeaderRewriteToRequest(req, relaycommon.HeaderRewriteInput{
			ChannelSetting: a.info.ChannelSetting, LegacyOverride: a.info.HeadersOverride, APIKey: key,
		}); err != nil {
			return nil, err
		}
	}
	client, err := service.GetHttpClientWithProxyFallback(proxy, fallbackDirect)
	if err != nil {
		return nil, err
	}
	started := time.Now()
...
func (a *TaskAdaptor) BuildContentRequest(task *model.Task, artifactKey string, clientRequest channel.TaskArtifactClientRequest) (*channel.TaskContentRequest, error) {
	...
	auth, err := resolveAuth(a.plugin.Meta.Auth, a.info.ApiKey, proxy, a.info != nil && a.info.ChannelMeta != nil && a.info.ChannelSetting.ProxyFallbackDirect)
	if err != nil {
		return nil, err
	}
	ctx["auth"] = auth
...
func (a *TaskAdaptor) queryContext(task *model.Task, key, baseURL, proxy string) (map[string]any, error) {
	...
	auth, err := resolveAuth(a.plugin.Meta.Auth, key, proxy, a.info != nil && a.info.ChannelMeta != nil && a.info.ChannelSetting.ProxyFallbackDirect)
	if err != nil {
		return nil, err
	}
	ctx["auth"] = auth
...
func (a *TaskAdaptor) submitContext(c *gin.Context, info *relaycommon.RelayInfo) map[string]any {
	...
	if auth, err := resolveAuth(a.plugin.Meta.Auth, info.ApiKey, proxy, a.info != nil && a.info.ChannelMeta != nil && a.info.ChannelSetting.ProxyFallbackDirect); err == nil {
		ctx["auth"] = auth
		ctx["authHeader"] = auth["authHeader"]
		if a.plugin.Meta.Auth.Type == "" || a.plugin.Meta.Auth.Type == "none" || a.plugin.Meta.Auth.Type == "api_key" {
