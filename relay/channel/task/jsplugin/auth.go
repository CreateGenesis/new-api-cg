...
func resolveAuth(meta pluginruntime.AuthMeta, apiKey, proxy string, fallbackDirect ...bool) (map[string]any, error) {
	...
	token, err := acquireAccessToken(credentials, proxy, len(fallbackDirect) > 0 && fallbackDirect[0])
	if err != nil {
		return nil, err
	}
