...
func TestGeminiChatHandlerDoesNotEstimateMissingUsageTokens(t *testing.T) {
	t.Parallel()
	...
	require.Equal(t, 0, usage.BillingUsage.GeminiUsageMetadata.PromptTokenCount)
}

func TestGeminiStreamHandlerDoesNotEstimateMissingUsageTokens(t *testing.T) {
	gin.SetMode(gin.TestMode)
	c, _ := gin.CreateTestContext(httptest.NewRecorder())
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/chat/completions", nil)
...
func TestGeminiTextGenerationHandlerDoesNotEstimateMissingUsageTokens(t *testing.T) {
	t.Parallel()
